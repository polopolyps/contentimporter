...
/**
 * Implementation of {@link ContentImportHandler} that uses the Polopoly product code to do the imports. Currently these API:s are not open, so most
 * of the import code is copied into the Testbase project pending a refactoring of the product code. 
 *
 */
public class StandardContentImportHandler implements ContentImportHandler {

	protected static Logger LOGGER = Logger
			.getLogger(StandardContentImportHandler.class.getName());
	...
	protected static final String WARNING_RESOURCE_TYPE_NOT_SUPPORTED = "Resource type not supported: '%1$s'. Supported types are: '.content' and '.xml'";
				...
				if (isValidResource(resourceURL)) {
					...
					if (fileName.endsWith(".content")) {
						...
						try {
							...
							TextContentParser textContentParser = new TextContentParser(inputStream, resourceURL, fileName);
	...
	private boolean isValidResource(URL resourceURL) {
		...
		if (resourceURL == null 
				|| !(new File(resourceURL.getFile()).exists())) {
			LOGGER.log(Level.WARNING,
							String.format(WARNING_RESOURCE_FILE_NOT_FOUND,
							resourceURL != null ? resourceURL.toString() : null));
			return false;
			
		} else if (!resourceURL.toString().endsWith(".content") 
					&& !resourceURL.toString().endsWith(".xml")) {
			LOGGER.log(Level.WARNING,
					String.format(WARNING_RESOURCE_TYPE_NOT_SUPPORTED,
